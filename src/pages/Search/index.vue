        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li
              ...
              :key="index"
            >
              {{ prop.split(":")[1] }} <i @click="removeProperty(index)">×</i>
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    //根据属性搜索
    searchProperty(attr, attrVal) {
      let prop = `${attr.attrId}:${attrVal}::${attr.attrName}`;
      const isProps = this.searchParams.props.some((item) => item === prop);
      if (isProps) {
        return;
      }
    ...
    //排序
     changOrder(orderFlag){
       let orginFlag = this.orginFlag
       let orginType = this.orginType
       let newOrder = ''
       if(orderFlag === orginFlag){
         newOrder = `${orderFlag}:${orginType==='asc' ? 'desc':'asc'}`
       }else{
